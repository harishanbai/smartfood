...
const NotificationsPanel = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="absolute right-0 top-14 w-[min(22rem,calc(100vw-2rem))] glass-panel rounded-2xl p-4 shadow-2xl z-50 border border-slate-200/80 dark:border-white/10 bg-white/95 dark:bg-[#0E3C2B]/95 backdrop-blur-xl animate-fade-in">
      <div className="flex items-center justify-between pb-3 mb-3 border-b border-slate-200/80 dark:border-white/5">
        <div className="flex items-center gap-2">
          <Bell className="h-4 w-4 text-emerald-600 dark:text-accentGreen" />
          <h4 className="font-bold text-sm text-slate-900 dark:text-white">Notifications</h4>
        </div>
        <button 
          onClick={onClose}
          className="text-slate-400 hover:text-slate-900 dark:text-gray-400 dark:hover:text-white p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-white/5 transition-colors cursor-pointer"
