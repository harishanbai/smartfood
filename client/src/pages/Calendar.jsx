...
const Calendar = () => {
  ...
  const handleMarkHoliday = async (dateToMark) => {
    const targetDate = dateToMark || selectedDate;
    ...
    try {
      await holidayApi.markHoliday({ date: targetDate });
  ...
  const handleRemoveHoliday = async (dateToRemove) => {
    ...
    try {
      await holidayApi.removeHoliday(targetDate);
  ...
  const fetchMonthMenus = async () => {
    ...
    try {
      ...
      const [historyRes, holidayRes] = await Promise.all([
        menuApi.getHistory(monthStr),
        holidayApi.getHolidays(monthStr).catch(() => ({ data: [] }))
      ]);
      ...
      const holidayMap = {};
      const holidayData = Array.isArray(holidayRes?.data) ? holidayRes.data : [];
      ...
      const fetchTmrw = tomorrowStr.startsWith(monthStr) && !menuMap[tomorrowStr];
      const fetchToday = todayStr.startsWith(monthStr) && !menuMap[todayStr];
      ...
      if (tomorrowRes?.data && !tomorrowRes.data.isHoliday) {
        menuMap[tomorrowStr] = tomorrowRes.data;
      }
      if (todayRes?.data && !todayRes.data.isHoliday) {
            ...
            <div className="space-y-3 max-h-[calc(100vh-250px)] overflow-y-auto pr-1">
              <div className="p-4 rounded-2xl bg-gradient-to-br from-gold-500/15 via-white/5 to-transparent border border-gold-500/40 text-center flex flex-col items-center justify-center space-y-2">
                ...
                <h4 className="text-sm sm:text-base font-bold text-white mb-0">
                  {holidays[selectedDate]?.name || t('holiday.title')}
                </h4>
                {holidays[selectedDate]?.name_ta && (
              ...
              <div className="pt-3 border-t border-white/5 text-[10px] text-gray-500 flex justify-between font-mono">
                ...
                <span className="text-gold-400 font-semibold">Status: Non-Working</span>
              </div>
            </div>
          ) : selectedDayMenu ? (
              ...
              {(() => {
                ...
                return (
                  <div className="p-4 rounded-2xl bg-white/3 border border-white/5 space-y-3">
                    ...
                    <div>
                      <span className={`inline-flex items-center gap-1 text-[9px] px-2.5 py-0.5 rounded-full font-bold uppercase tracking-wider ${isNonVeg
